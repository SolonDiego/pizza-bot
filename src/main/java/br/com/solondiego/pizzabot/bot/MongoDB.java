...
public class MongoDB {
    ...
    public static void insertNewUserId(String userId) {
        ...
        try {
            if (!userExists(userId)) {
                InsertOneResult result = customersCollection.insertOne(new Document().append("_id", userId).append(ORDER_STATE, OrderState.PIZZA_SELECTION.toString())
                        .append(PIZZA_TYPE, "").append(PIZZA_SIZE, "").append(DRINK, "").append(ADDRESS, ""));
    ...
    public static String getFieldValue(String fieldName, String userChatId) {
        ...
        Document result = customerCollection.find(eq("_id", userChatId)).projection(fields(include(fieldName), excludeId())).first();
